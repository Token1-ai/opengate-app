/**
 * OpenGate Battleship Oracle — Base regular table
 * File: api/oracle-base.js
 *
 ...
 *     BNB, offset by BASE_OFFSET so contract_game_id ranges never
 ...
 * that wallet just also needs a small ETH balance for Base gas.
 */

const { ethers } = require('ethers');
const { createClient } = require('@supabase/supabase-js');

const BATTLESHIP_CONTRACT = '0x22597403bFa0982803Be28D070a14b6E45700dc0'; // BattleshipBase regular (Base)
...
// under load.
const RPCS = [
  'https://base-mainnet.g.alchemy.com/v2/beDxJpk3FwajrKr5SakSi', // dedicated key, tried first
...
const BASE_OFFSET = 5000000000; // keeps this table's DB ids out of BNB (0), BNB Gold (2B), Polygon (3B), Polygon Gold (4B)
...
module.exports = async function handler(req, res) {
  ...
  const DB_GID = BASE_OFFSET + Number(gameId);
  ...
  try {
    ...
    // Defense-in-depth: the contract itself already enforces a $50 cap
    // at createGame time, but refuse to service anything over it here
    // too — same belt-and-suspenders pattern as the BNB oracle.
    const MAX_BET = 50n * 10n ** 6n; // stablecoins are 6 decimals
    ...
    console.error('[Oracle-Base Error]', err);
